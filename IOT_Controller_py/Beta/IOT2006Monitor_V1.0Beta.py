...
LastEditTime: 2021-03-24 07:13:13
'''
...
def Get_Ip_List():
    if os.path.exists('Ip_List.txt'):
        with open(r'Beta\Ip_List.txt', 'r', encoding="utf-8") as file:
            Ip_List = file.read()
        return Ip_List
    else:
        with open(r'Beta\Ip_List.txt', 'w', encoding="utf-8") as file:
            pass
        return '[]'
...
def Ui():
    ...
    Frame_Left_Layout = sg.Frame("Left",
                                        [[sg.Column([
                                                        [sg.Text('Local IP:')],
                                                        [sg.Input('192.168.10.110', key='-ip-', readonly=True)],
