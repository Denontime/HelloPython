...
col2 = Column([[Frame('Accounts:', [[Column([[Listbox(['Account ' + str(i) for i in range(1, 16)], key = '-ACCT-LIST-', size = (15, 20)),],
...
col1 = Column([
    ...
    [Frame('Information:', [[Text(), Column([[Text('Account:')],
                                     [Input(List[List_index], key='-ACCOUNT-IN-', size=(19, 1))],
                                     [Text('User Id:')],
...
while True:
    ...
    if event == '-left-':
        if List_index == 0:
            ...
            List_index -= 1
        print(List_index)
        left_elem.update(List[List_index])
